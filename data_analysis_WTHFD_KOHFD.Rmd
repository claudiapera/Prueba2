---
title: "data_analysis_WTHFD_KOHFD"
...
pks <- rMSI2::LoadPeakMatrix("/home/guest/msidata2/TFG_Claudia/Procesado/analysisWTHFD_KOHFD/merged-peakmatrix.pkmat")
...
df <- data.frame(mass = pks$mass,
                 int_WTHFD = rep(NA, length(pks$mass)),#create as many of these columns as samples you have
                 int_KOHFD = rep(NA, length(pks$mass)),
...
{
  df$int_KOHFD[i] <- mean(pks$intensity[which(pks$region==levels(pks$region)[1]), i]) 
  df$int_WTHFD[i] <- mean(pks$intensity[which(pks$region==levels(pks$region)[2]), i])
  df$int[i] <- mean(pks$intensity[i])
}

df_long<- gather(df, region, intensity, int_WTHFD:int_KOHFD, factor_key=TRUE)
box_names <- c(int_WTHFD = new_names[1], int_KOHFD = new_names[2])
...
s <- ggplot() +
    geom_segment(aes(x=df$mass, y=df$int_WTHFD, xend = df$mass, yend = 0), colour = "red") + 
    geom_segment(aes(x=df$mass, y=df$int_KOHFD*(-1), xend = df$mass, yend = 0), colour = "blue") +
    ...
    annotate(geom = "text", label = var1_title, color = "red", x = 500, y = max(df$int_HFD)) + 
    annotate(geom = "text", label = var2_title, color = "blue", x = 500, y = min(df$int_CT * (-1)))
