...
df <- data.frame(mass = pks$mass,
                 int_HFD = rep(NA, length(pks$mass)),#create as many of these columns as samples you have
                 int_CT = rep(NA, length(pks$mass)),
...
{
  df$int_CT[i] <- mean(pks$intensity[which(pks$region==levels(pks$region)[1]), i]) 
  df$int_HFD[i] <- mean(pks$intensity[which(pks$region==levels(pks$region)[2]), i])
  df$int[i] <- mean(pks$intensity[i])
}

df_long<- gather(df, region, intensity, int_HFD:int_CT, factor_key=TRUE)
box_names <- c(int_HFD = new_names[1], int_CT = new_names[2])
...
s <- ggplot() +
    geom_segment(aes(x=df$mass, y=df$int_HFD, xend = df$mass, yend = 0), colour = "red") + 
    geom_segment(aes(x=df$mass, y=df$int_CT*(-1), xend = df$mass, yend = 0), colour = "blue") +
...
print("m/z that are significantly different between HFD and Control")
